...
(async function () {
    'use strict';
    const DATA_URL = 'https://files.benfex.net/uploads/flynas/3rd-OCT-Agency.json';
    const $ = window.jQuery;
    ...
    const DEFAULT_AGENCY = 'Tamair'; // Default agency name
    ...
    async function fillBooking(booking) {
        ...
        // ---- Default or provided agency ----
        const agencyName = booking.agency_name || DEFAULT_AGENCY;
        $('#agencySelect option').filter(function () {
            return $(this).text().toLowerCase().includes(agencyName.toLowerCase());
        }).prop('selected', true).trigger('change');
        $('#agencyNameDisplay').val(agencyName);
    ...
    try {
        ...
        // Move to next booking using Ctrl + Alt + N
